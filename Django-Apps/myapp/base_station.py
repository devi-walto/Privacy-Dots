######################################################
# File was programmed by , Devin, Austin, and Carson #
######################################################
...
# The Django server was originally supposed to run on the Rpi but the Rpi would freeze or crash everytime we tried to run it. 
# The same thing happened with the webapp.
# So we dicided to have it all run on Diego's laptop.
# Which is why there is some code that is specificly for the Rpi.
...
#############################################################################################################            
    ...
    def launch_webapp(self): # Function to launch the webapp
        ...
        try:
            web_app_path = "/Users/arreo/Desktop/PrivacyDots" ####### @Diego: Please update this path to the actual path of the webapp
            entry_file = "App.js" ####### @Diego: Please update this to the actual entry file of the webapp
    ...
    # Node functions
    # This funciton was not used becsause the Pi was not able to donwload the bluepy for some reason, so we switched to Wifi.
    # Unfortunalty the connecting the ESP32 to campus WiFI was not possible so we had to hadrwire the nodes to the breadboard on the Rpi
    # However the function bellow works correctly to connect nodes via bluetooth
